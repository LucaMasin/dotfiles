...
- `.config/opencode/opencode.jsonc` adds a `poteto` primary agent beside `build`/`plan` with the same `subagent:implement` permission as `build` (`openai/gpt-5.6-sol` at medium reasoning). `build` itself stays untouched.
